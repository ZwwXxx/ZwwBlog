    ...
    <el-row class="articleLayOut">
      ...
      <el-col :xs="24" :sm="18">
        <div v-show="!loading" class="leftSide">
          ...
          <div class="articleBody themeBg">
            ...
            <v-md-preview :text="article.content" ref="articleContent" class="articleContent themeText" @copy-code-success="handleCopyCodeSuccess"></v-md-preview>
            <!--</div>-->
          </div>
      ...
      <el-col :xs="0" :sm="6">
        ...
        <div v-show="!loading" class="rightSide">
          <!--<sticky-vue :offset-top="100">-->
          <div class="directory themeBg" ref="directory" :style="fixedCata">
            <div class="themeText">文章目录</div>
            <div class="directoryItem">
              <ul class="title" v-show="initSuccess">
                <li v-for="(anchor, index) in titles" :key="index">
                  <!--<a :href="'#'+item.id">-->
                  <!--    {{ item.title }}-->
                  <!--</a>-->
                  <a @click="handleAnchorClick(anchor)" :id="anchor.id" :style="{ paddingLeft: anchor.level * 10 + 'px' }" style="color: var(--text-color)">
                    {{ anchor.title }}
                  </a>
                </li>
              </ul>
      ...
      <!--复制代码弹框提示信息-->
      <el-alert title="复制成功" type="success" :closable="false" show-icon v-show="alertVisible" class="alertBox" style="width: 300px" ref="alertBox"> </el-alert>
    </el-row>
  </div>
</template>
...
import StickyVue from '@/components/stickyVue.vue'
...
export default {
  name: 'ArticleDetail',
  components: { StickyVue, BgBoard, CommentInfoInput, Comment, Loading, WangEditor },
  watch: {
  ...
  methods: {
    ...
    generateCatalog() {
      ...
      this.$nextTick(() => {
        ...
        // setTimeout(() => {
        //   // 初始化高亮
        //   // 不要这个高亮了，不然监听滚动事件太快会产生闪一下的结果，导致点击高亮鬼畜了
        //   this.setClickListen()
        // }, 50)
        this.titles = titles
      })
    ...
    handleScroll() {
      ...
      for (let i = 0; i < this.titlesDoms.length; i++) {
        ...
        if (rect.top < 70 && rects[i + 1] && rects[i + 1].top > document.documentElement.clientHeight) {
          this.highlight(titleDom)
          break
        }
      }
